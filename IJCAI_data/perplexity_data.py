...
def averaged_perplexity(lines: list[str]):
    ...
    lines: list[list[tuple[str,str]]] = [[iter.split('/') for iter in line] for line in lines]
    ...
    # print(f"failed {failed} out of {failed + len(values)}")
    print("break")
    values = [sum(iter)/len(iter) if len(iter) > 0 else 0 for iter in values]

    return [values]

files = {
    "nn" : "IJCAI_data/nn/perplexity.txt",
    "cpbp" : "IJCAI_data/cpbp/perplexity.txt",
    "nncp" : "IJCAI_data/nncp/perplexity.txt",
    "nncpbp_0.5" : "IJCAI_data/nncpbp/0.5_perplexity.txt",
    "nncpbp_1.0" : "IJCAI_data/nncpbp/1.0_perplexity.txt",
    "nncpbp_1.5" : "IJCAI_data/nncpbp/1.5_perplexity.txt"
}
